...
public interface Users_followingDao {
    ...
    public List<UsersFollowing> selectUserFollowInformation(String follow_user);//查询用户关注的信息

    @Select("SELECT * FROM Users_following WHERE be_followed=#{followed_user}")
    public List<UsersFollowing> selectUserFollowedInformation(String followed_user);//查询用户的粉丝信息
